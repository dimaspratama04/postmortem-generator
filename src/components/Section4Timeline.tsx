  ...
  return (
    <SectionCard num="04" title="Incident Timeline" id="section-4">
      <p style={{ fontSize: 12, color: 'var(--color-text-dim)', marginBottom: 16, fontStyle: 'italic' }}>
        List all key events chronologically. Include alert trigger, investigation steps, mitigation actions, and resolution.
      </p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '100px 180px 1fr 36px', gap: 8, padding: '6px 8px', marginBottom: 4 }}>
        {['Time', 'Actor', 'Event / Action', ''].map((h, i) => (
          <span key={i} className="field-label" style={{ marginBottom: 0 }}>{h}</span>
        ))}
      </div>
      ...
      {data.timeline.map((entry, index) => (
        ...
        >
          <input
            ...
            value={entry.time}
            onChange={e => updateRow(entry.id, 'time', e.target.value)}
            placeholder="HH:MM"
            style={{ fontSize: 13 }}
          />
          <input
            className="field-input"
            value={entry.actor}
            onChange={e => updateRow(entry.id, 'actor', e.target.value)}
            placeholder="Actor / Team"
            style={{ fontSize: 13 }}
          />
          <input
            className="field-input"
            value={entry.event}
            onChange={e => updateRow(entry.id, 'event', e.target.value)}
          ...
          <button
            ...
            disabled={data.timeline.length <= 1}
            style={{ padding: '6px', justifyContent: 'center', opacity: data.timeline.length <= 1 ? 0.3 : 1 }}
            title="Remove row"
          >
            <Trash2 size={14} />
          </button>
